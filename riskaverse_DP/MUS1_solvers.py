...
class SNMI:
    ...
    def solve(self, mdp, v_0, verbose=True):
        ...
        G = matrix(np.vstack((-np.eye(mdp.n), np.eye(mdp.n))))
        ...
        for a in range(mdp.m):
            ...
            for s in range(mdp.n):
                c = matrix(-v_0)
                h = matrix(np.concatenate((np.zeros(mdp.n), (1+self.alpha) * P[s, :]))[:, np.newaxis])
    ...
    def solve(self, mdp, v_0, verbose=True, warmstarting=True):
        ...
        while (res_[-1])>self.tol1:
            ...
            if warmstarting:
                ...
                for s in range(mdp.n):
                    c = matrix(-v_inner)
                    h = matrix(np.concatenate((np.zeros(mdp.n), (1+self.alpha) * P_pi[s, :]))[:, np.newaxis])
                    sol = solvers.lp(c, G, h, A, b)
